...
////////////////////////////////////////////////////////////////////////////////
// Same design as the Linux CMEvent port, but the counting semaphore is a GCD
// dispatch_semaphore: macOS does not support unnamed POSIX semaphores
// (sem_init / sem_timedwait). dispatch_semaphore timeouts are relative and
// based on a monotonic clock, so they are immune to system time changes.
...
uint32_t get_signal(c_event::s_impl *ip_impl)
{
    ...
    do
    {
        ...
        if(ip_impl->mp_event_cur->mi_counter)
        {
            ...
            {
                dispatch_semaphore_signal(ip_impl->mh_sem); // keep manual events level-triggered
            }
            else
            {
...
c_event::~c_event()
{
    if(mp_impl->mb_init)
    {
        ...
        if(mp_impl->mh_sem)
        {
            dispatch_release(mp_impl->mh_sem);
            mp_impl->mh_sem = nullptr;
        }
    }
...
bool c_event::init(uint8_t iu_count, const e_type *ip_types)
{
    ...
    mp_impl->mh_sem = dispatch_semaphore_create(0);
    if(nullptr == mp_impl->mh_sem)
    {
        goto l_lblExit;
    }
    else
...
bool c_event::set(uint32_t iu_id)
{
    ...
    dispatch_semaphore_signal(mp_impl->mh_sem);
...
bool c_event::clr(uint32_t iu_id)
{
    ...
    if(mp_impl->mp_events[iu_id].mi_counter)
    {
        ...
        dispatch_semaphore_wait(mp_impl->mh_sem, DISPATCH_TIME_NOW); // decrease semaphore
...
uint32_t c_event::wait()
{
    ...
    dispatch_semaphore_wait(mp_impl->mh_sem, DISPATCH_TIME_FOREVER);
...
{
    uint32_t        lu_return = c_event::timeout;
    dispatch_time_t lu_time = (0 == iu_timeout_ms)
                                  ? DISPATCH_TIME_NOW
                                  : dispatch_time(DISPATCH_TIME_NOW, (int64_t)iu_timeout_ms * (int64_t)NSEC_PER_MSEC);

    if(0 == dispatch_semaphore_wait(mp_impl->mh_sem, lu_time))
    {
        pthread_mutex_lock(&mp_impl->ms_mutex);
        lu_return = get_signal(mp_impl);
        pthread_mutex_unlock(&mp_impl->ms_mutex);
