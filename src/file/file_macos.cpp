...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Map kit open parameters to POSIX open() flags. Returns false when the
// combination is invalid (e.g. both hint flags set, or neither read nor
// write).
...
c_file::~c_file()
{
    if(nullptr != mp_impl)
    {
        // Best-effort close without fsync; caller who needs durability must
        // call close(true) or sync() explicitly before dropping the object.
        if(mp_impl->mi_fd >= 0)
        {
            ::close(mp_impl->mi_fd);
            mp_impl->mi_fd = -1;
...
bool c_file::open(const std::filesystem::path &ir_path, e_file_open_mode ie_mode, uint32_t iu_flags)
{
    ...
    // 0644 keeps parity with typical umask defaults.
    ...
    // Post-open hint / cache adjustments (best-effort; failures do not fail
    // open() itself).
    if(has_flag(iu_flags, e_ff_no_buffering))
    {
        fcntl(li_fd, F_NOCACHE, 1);
    }
    if(has_flag(iu_flags, e_ff_hint_sequential))
    {
        fcntl(li_fd, F_RDAHEAD, 1);
    }
    if(has_flag(iu_flags, e_ff_hint_random))
    {
        fcntl(li_fd, F_RDAHEAD, 0);
    }
...
size_t c_file::write(const void *ip_buf, size_t iz_size, bool ib_flush)
{
    ...
    while(lz_done < iz_size)
    {
        ...
        {
            // POSIX allows write() to return 0 only for a zero-length
            // request; if we asked for >0 and got 0, treat as a stall to
...
bool c_file::preallocate(uint64_t iu_size)
{
    ...
    // F_PREALLOCATE reserves on-disk blocks; try contiguous first, fall back
    // to a best-effort layout on fragmentation.
    fstore_t ls_store    = {};
    ls_store.fst_flags   = F_ALLOCATECONTIG;
    ls_store.fst_posmode = F_PEOFPOSMODE;
    ls_store.fst_offset  = 0;
    ls_store.fst_length  = static_cast<off_t>(iu_size);

    if(fcntl(mp_impl->mi_fd, F_PREALLOCATE, &ls_store) == -1)
    {
        ls_store.fst_flags = F_ALLOCATEALL;
        if(fcntl(mp_impl->mi_fd, F_PREALLOCATE, &ls_store) == -1)
        {
            mp_impl->mi_last_err = errno;
            return false;
    ...
    // Extend the logical size to match — callers expect get_size() >= iu_size
    // after a successful preallocate() to align with the Linux behaviour of
